...
export class FloorManager {
    ...
    schedule(stage, delayMs) {
        ...
        this.timer = setTimeout(async () => {
            ...
            if (generation !== this.generation || this.disposed || this.resultStarted || this.cueCount >= this.maxCues)
                return;
            const cue = await resolveWithin(generated, 500, controller.signal)
                ?? (this.resolveCue === undefined ? composeFloorCue(request) : '嗯，我先看一下。');
            if (generation !== this.generation || this.disposed || this.resultStarted || controller.signal.aborted)
                return;
 ...
 */
export function composeFloorCue(context) {
    ...
    for (let offset = 0; offset < candidates.length; offset++) {
        ...
        if (!context.previousCues.includes(cue))
            return cue;
    }
...
function classifyIntent(task) {
    ...
    if (/(查|搜索|天气|新闻|资料|附近|哪里|几点|价格|最新)/.test(normalized))
        return 'lookup';
    if (/(对比|比较|选哪个|哪个好|区别|优缺点)/.test(normalized))
        return 'compare';
    if (/(写|改|修|安装|创建|生成|处理|执行|操作|发布|上传)/.test(normalized))
        return 'action';
    if (/(计划|规划|分析|为什么|怎么办|怎么练|怎么做|建议|推理)/.test(normalized))
        return 'analysis';
    return 'generic';
}

	...
	factory: (require) => {
		...
		*/
		var FloorManager = class {
			...
			generation = 0;
			requestAbort;
			constructor(delayMs, emit, timings = {}) {
				this.delayMs = delayMs;
				this.emit = emit;
				this.progressDelayMs = timings.progressDelayMs ?? 3500;
				this.longWaitMs = timings.longWaitMs ?? 7e3;
			...
			*/
			...
			schedule(stage, delayMs) {
				...
				let generated;
				try {
					generated = this.resolveCue === void 0 ? void 0 : Promise.resolve(this.resolveCue(request, controller.signal)).catch(() => void 0);
				} catch {
					generated = Promise.resolve(void 0);
				}
				this.timer = setTimeout(async () => {
					this.timer = void 0;
					if (generation !== this.generation || this.disposed || this.resultStarted || this.cueCount >= this.maxCues) return;
					const cue = await resolveWithin(generated, 500, controller.signal) ?? (this.resolveCue === void 0 ? composeFloorCue(request) : "嗯，我先看一下。");
					...
					this.cueCount++;
					this.emit(cue);
					if (!this.disposed && !this.resultStarted && this.cueCount < this.maxCues) this.schedule("long-wait", this.longWaitMs);
				}, Math.max(0, delayMs));
			}
			cancelTimer() {
		...
		async function resolveWithin(pending, timeoutMs, signal) {
		...
		*/
		function composeFloorCue(context) {
		...
		function cueCandidates(stage, intent, topic) {
		...
		function extractTopic(task) {
			const compact = task.replace(/<!--[\s\S]*?-->/g, " ").replace(/https?:\/\/\S+/gi, " ").replace(/\b(?:sk|key|token)-[a-z0-9_-]+\b/gi, " ").replace(/\b(?:gh[pousr]_[a-z0-9]+|github_pat_[a-z0-9_]+|A(?:KI|SI)A[0-9A-Z]{16})\b/gi, " ").replace(/\bBearer\s+\S+/gi, " ").replace(/\b(?:password|passwd|secret|token|api[_-]?key)\s*[:=]\s*\S+/gi, " ").replace(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi, " ").replace(/\b(?:[a-f0-9]{32,}|[a-z0-9+/]{24,}={0,2})\b/gi, " ").replace(/[\r\n\t]+/g, " ").replace(/\s+/g, "").replace(/^(?:请|麻烦|你能不能|能不能|可不可以|帮我|给我|我想|想让你)+/g, "").replace(/^(?:查一下|查查|搜索|查|看看|看一下|分析一下|分析|比较一下|对比一下|处理一下|处理)+/g, "").replace(/[，。！？，.!?;:；："'“”‘’`<>{}[\]()/\\|]+/g, "").slice(0, 18);
		...
		function hasCustomPrefs(prefs) {
			return prefs.provider !== "qwen" || prefs.qwenWorkspaceId !== "" || prefs.qwenModel !== DEFAULTS.qwenModel || prefs.qwenVoice !== DEFAULTS.qwenVoice || prefs.qwenAsrModel !== DEFAULTS.qwenAsrModel || prefs.qwenTtsModel !== DEFAULTS.qwenTtsModel || prefs.qwenTtsVoice !== DEFAULTS.qwenTtsVoice || prefs.qwenVadThreshold !== DEFAULTS.qwenVadThreshold || prefs.qwenSilenceMs !== DEFAULTS.qwenSilenceMs || prefs.qwenMergeMs !== DEFAULTS.qwenMergeMs || prefs.voiceDraftAutoSend !== DEFAULTS.voiceDraftAutoSend || prefs.voiceDraftDwellMs !== DEFAULTS.voiceDraftDwellMs || prefs.voiceDraftAllowWithoutVoiceprint !== DEFAULTS.voiceDraftAllowWithoutVoiceprint || prefs.voiceDraftSensitiveDeny !== DEFAULTS.voiceDraftSensitiveDeny || prefs.floorDelayMs !== DEFAULTS.floorDelayMs || prefs.floorComposerEnabled !== DEFAULTS.floorComposerEnabled || prefs.qwenFloorModel !== DEFAULTS.qwenFloorModel || prefs.openaiFloorModel !== DEFAULTS.openaiFloorModel || prefs.voiceprintEnabled !== DEFAULTS.voiceprintEnabled || prefs.voiceprintThreshold !== DEFAULTS.voiceprintThreshold || prefs.openaiModel !== DEFAULTS.openaiModel || prefs.openaiVoice !== DEFAULTS.openaiVoice || prefs.instructions !== DEFAULTS.instructions;
		}
		/**
		...
		*/
		...
		function sanitize(value) {
			const qwenMergeMs = numberInRange(value.qwenMergeMs, 100, 5e3, DEFAULTS.qwenMergeMs);
			return {
				...
				voiceDraftDwellMs: Math.max(qwenMergeMs, numberInRange(value.voiceDraftDwellMs, 500, 6e3, DEFAULTS.voiceDraftDwellMs)),
		...
		//#endregion
		//#region src/client/floor-composer.ts
		const URL = "/dsh-realtime-voice/floor-compose";
		const resolveDynamicFloorCue = async (request, signal) => {
			const prefs = loadPrefs();
			if (!prefs.floorComposerEnabled) return void 0;
			...
			return typeof body.cue === "string" ? body.cue : void 0;
		};
		//#endregion
...
`.trim();
		...
		//#endregion
		//#region src/client/voiceprint.ts
		...
		var VoiceprintCapture = class {
			...
			push(frame) {
				...
				while (this.preRollSamples > PRE_ROLL_SAMPLES && this.preRoll.length > 1) {
					...
					if (removed !== void 0) this.preRollSamples -= removed.length;
				}
				const active = this.active;
				if (active === void 0 || active.samples >= MAX_SAMPLES) return;
			...
			start(itemId) {
				...
				this.active = {
					itemId,
					frames,
					samples: frames.reduce((sum, frame) => sum + frame.length, 0)
				};
			}
			stop(itemId) {
				const active = this.active;
				...
				if (key === "" || active.samples < SAMPLE_RATE) return;
				...
				while (this.completed.size > 4) this.completed.delete(this.completed.keys().next().value);
			}
			takeBase64(itemId) {
				const samples = this.completed.get(itemId);
				if (samples === void 0) return void 0;
				this.completed.delete(itemId);
				return pcmBase64(samples);
			}
			discard(itemId) {
				if (itemId !== "") this.completed.delete(itemId);
				if (this.active?.itemId === itemId) this.active = void 0;
			}
			clear() {
				this.preRoll = [];
				this.preRollSamples = 0;
		...
		async function getVoiceprintStatus() {
			try {
				const response = await fetch("/dsh-realtime-voice/status", {
					cache: "no-store",
					headers: { accept: "application/json" }
				});
				const body = await response.json();
		...
		async function checkVoiceprint(operation, audio) {
			try {
				...
				if (operation === "enroll") return body.enrolled === true ? { status: "enrolled" } : {
					status: "unavailable",
					error: "声纹服务未确认录入"
				};
		...
		var QwenPipelineConnection = class {
			...
			voiceprintEpoch = 0;
			voiceprintHandledItems = /* @__PURE__ */ new Set();
			...
			async handleAsr(raw) {
				...
				if (type === "conversation.item.input_audio_transcription.completed") {
					...
					if (this.prefs.voiceprintEnabled && itemId !== "") {
						...
						while (this.voiceprintHandledItems.size > 128) this.voiceprintHandledItems.delete(this.voiceprintHandledItems.values().next().value);
					}
					...
					await this.callbacks.onTranscript?.(transcript, {
						capturedWhileBusy,
						voiceprint: this.prefs.voiceprintEnabled ? "approved" : void 0
					});
				}
			...
			beginVoiceprintCheck(itemId) {
				...
				if (audio === void 0) return Promise.resolve({
					status: "unavailable",
					error: "有效人声不足一秒"
				});
				if (!this.voiceprintConfigured) return Promise.resolve({
					status: "unavailable",
					error: "腾讯云声纹凭据未配置"
				});
				return checkVoiceprint(this.voiceprintEnrolled ? "verify" : "enroll", audio).then((result) => {
			...
			async dispatchVoiceprintTranscript(itemId, transcript, capturedWhileBusy) {
				...
				if (voiceprint.status !== "approved") {
					const detail = voiceprint.status === "rejected" ? `声纹未通过（${Math.round(voiceprint.score)}分）；文字已保留，可手动发送` : "声纹暂不可用；文字已保留，可手动发送";
		...
		*/
		...
		var VoiceController = class {
			...
			bindDraft(target) {
				...
				} else if (currentRev !== void 0 && this.boundDraftRev !== void 0 && currentRev !== this.boundDraftRev) {
					if (!this.pluginDraftWritePending) this.disarmDraftAutoSend("输入框已由你修改；自动发送已取消");
					this.boundDraftRev = currentRev;
					this.pluginDraftWritePending = false;
					if (this.draftAutoSendLease !== void 0) this.draftAutoSendLease.expectedDraftRev = currentRev;
				} else if (this.boundDraftRev === void 0) this.boundDraftRev = currentRev;
			...
			async bufferTranscript(source, transcript, meta = {}) {
				...
				else if (meta.voiceprint === "approved" && this.transcriptVoiceprint === void 0) this.transcriptVoiceprint = "approved";
				...
				this.transcriptTimer = setTimeout(() => {
					...
					if (combined !== "") this.turns.enqueue(() => this.composerOnly ? wasBusy ? this.stageComposerTranscript(source, combined, {
						capturedWhileBusy,
						voiceprint
					}) : this.submitComposerTranscript(source, combined) : wasBusy ? this.handleBusyTranscript(source, combined) : this.handleTranscript(source, combined));
			...
			submitComposerTranscript(source, transcript) {
				if (this.connection !== source) return;
				if (this.draftTarget?.submit === void 0) {
			...
			submitBoundDraft(source) {
				...
				if (target?.submit === void 0 || submittedTask === "") return;
				this.disarmDraftAutoSend();
			...
			beginObservedTurn(source, harnessTurn) {
				...
				observed.floor = new FloorManager(floorPrefs.floorDelayMs, enqueueSpeech, { resolveCue: floorPrefs.floorComposerEnabled ? resolveDynamicFloorCue : void 0 });
			...
			async finishObservedTurn(harnessTurn, result) {
				...
				try {
					...
					this.setTurnPhase(turnId, "listening");
					this.setState("listening", isBargeInError(observed.speechError) ? "播报已打断；识别文字保留在输入框" : this.hasPendingDraft() ? "Harness 已完成；输入框里的后续语音正在等待发送" : "Harness 已完成；继续说将自动处理");
			...
			async runHarnessTurn(source, task, turnId, taskAbort) {
				...
				const floorPrefs = loadPrefs();
				const floor = new FloorManager(floorPrefs.floorDelayMs, enqueueSpeech, { resolveCue: floorPrefs.floorComposerEnabled ? resolveDynamicFloorCue : void 0 });
				floor.start(task);
				const result = await this.bridge.delegate(this.sessionId, task, taskAbort.signal, {
			...
			takeBufferedTranscript() {
				...
				this.transcriptVoiceprint = void 0;
				return combined;
			}
			...
			armDraftAutoSend(source, meta) {
				...
				const voiceprintAllowed = prefs.voiceprintEnabled ? meta.voiceprint === "approved" : prefs.voiceDraftAllowWithoutVoiceprint;
				const continuation = this.draftAutoSendLease?.source === source;
				const task = this.boundDraft.trim();
				if (!prefs.voiceDraftAutoSend || voiceprintBlocked || !voiceprintAllowed || !meta.capturedWhileBusy && !continuation || prefs.voiceDraftSensitiveDeny && isSensitiveDraft(task) || isExplicitCancel(task)) {
					this.disarmDraftAutoSend();
					return;
				}
				...
				this.draftAutoSendLease = {
					...
					expectedDraftRev: this.pluginDraftWritePending ? void 0 : this.boundDraftRev,
			...
			isDraftAutoSendSafe(lease) {
				return this.connection === lease.source && this.composerOnly && this.turns.phase === "listening" && this.taskAbort === void 0 && !this.nativeSubmitPending && !this.draftSpeechActive && (this.draftTarget?.getPhase?.() ?? "plain") === "plain" && this.draftTarget?.submit !== void 0 && lease.expectedDraft.trim() !== "";
			}
			pauseDraftAutoSend() {
				if (this.draftAutoSendTimer !== void 0) clearTimeout(this.draftAutoSendTimer);
				this.draftAutoSendTimer = void 0;
				if (this.draftCommitTimer !== void 0) clearTimeout(this.draftCommitTimer);
				this.draftCommitTimer = void 0;
				if (this.draftSpeechResumeTimer !== void 0) clearTimeout(this.draftSpeechResumeTimer);
				this.draftSpeechResumeTimer = void 0;
			}
			...
			handleSpeechEnd(source) {
				...
				this.draftSpeechResumeTimer = setTimeout(() => {
					this.draftSpeechResumeTimer = void 0;
					if (this.validDraftAutoSendLease(generation) !== void 0) this.scheduleDraftAutoSend();
				}, resumeMs);
			}
			validDraftAutoSendLease(generation) {
				const current = this.draftAutoSendLease;
				const target = this.draftTarget;
				if (current === void 0 || current.generation !== generation || !this.isDraftAutoSendSafe(current) || target?.getDraft() !== current.expectedDraft || current.expectedDraftRev !== void 0 && target.getDraftRev?.() !== current.expectedDraftRev || this.boundDraft !== current.expectedDraft) return void 0;
				return current;
			}
			disarmDraftAutoSend(detail) {
				const hadLease = this.draftAutoSendLease !== void 0;
				this.pauseDraftAutoSend();
				this.draftAutoSendLease = void 0;
				this.draftAutoSendGeneration++;
				if (detail !== void 0 && hadLease && this.connection !== void 0 && this.turns.phase === "listening") this.setState("listening", detail);
			}
			updateDraftAutoSendStatus() {
				const lease = this.draftAutoSendLease;
				if (lease !== void 0 && this.isDraftAutoSendSafe(lease)) {
		...
		function SettingsCard() {
			...
			const [voiceprint, setVoiceprint] = (0, react.useState)({
				configured: false,
				enrolled: false
			});
			const [voiceprintMessage, setVoiceprintMessage] = (0, react.useState)("");
			(0, react.useEffect)(() => {
				if (!open || prefs.provider !== "qwen") return;
				getVoiceprintStatus().then(setVoiceprint);
			}, [
				open,
				prefs.provider,
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					prefs.provider === "qwen" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								type: "checkbox",
								checked: prefs.voiceDraftAutoSend,
								onChange: (e) => updatePrefs({ voiceDraftAutoSend: e.currentTarget.checked })
							})
						}),
						prefs.voiceDraftAutoSend && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
								label: `输入框停留(ms，≥${Math.max(500, prefs.qwenMergeMs)})`,
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									style: styles.input,
									type: "number",
									min: Math.max(500, prefs.qwenMergeMs),
									max: 6e3,
									step: 100,
									value: prefs.voiceDraftDwellMs,
									onChange: (e) => updatePrefs({ voiceDraftDwellMs: e.currentTarget.valueAsNumber })
								})
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
								label: "无声纹时允许",
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									type: "checkbox",
									checked: prefs.voiceDraftAllowWithoutVoiceprint,
									onChange: (e) => updatePrefs({ voiceDraftAllowWithoutVoiceprint: e.currentTarget.checked })
								})
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
								label: "敏感指令留待确认",
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									type: "checkbox",
									checked: prefs.voiceDraftSensitiveDeny,
									onChange: (e) => updatePrefs({ voiceDraftSensitiveDeny: e.currentTarget.checked })
								})
							})
						] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								type: "checkbox",
								checked: prefs.voiceprintEnabled,
								onChange: (e) => updatePrefs({ voiceprintEnabled: e.currentTarget.checked })
							})
						}),
						prefs.voiceprintEnabled && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
							label: "声纹通过分数",
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								style: styles.input,
								type: "number",
								...
								max: 100,
								step: 1,
								value: prefs.voiceprintThreshold,
								onChange: (e) => updatePrefs({ voiceprintThreshold: e.currentTarget.valueAsNumber })
							})
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
							label: "声纹状态",
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: !voiceprint.configured ? "缺少腾讯云凭据" : voiceprint.enrolled ? "已录入" : "待录入：重开语音后，说第一句至少 1 秒的人声" }),
								voiceprint.enrolled && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									style: {
										...styles.input,
										marginLeft: 8,
								...
								voiceprintMessage && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									style: {
										opacity: .66,
										fontSize: 12,
										marginTop: 4
									},
									children: voiceprintMessage
								})
							] })
						})] })
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							type: "checkbox",
							checked: prefs.floorComposerEnabled,
							onChange: (e) => updatePrefs({ floorComposerEnabled: e.currentTarget.checked })
						})
					}),
					prefs.floorComposerEnabled && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
						label: "接场轻量模型",
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							style: styles.input,
							value: prefs.provider === "qwen" ? prefs.qwenFloorModel : prefs.openaiFloorModel,
							onChange: (e) => prefs.provider === "qwen" ? updatePrefs({ qwenFloorModel: e.currentTarget.value }) : updatePrefs({ openaiFloorModel: e.currentTarget.value })
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
						label: "播报风格",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
						...
						children: [
							...
							"。空闲且输入框为空时，完整语句会立即交给 Harness；推理或播报期间的新语音会完整合并在原生输入框，播报结束并停留指定时间后再自动发送。继续说会重置计时，键盘编辑、粘贴、清空或手动发送会取消自动发送。声纹拒绝和敏感指令始终保留为手动确认；未配置声纹时需显式开启“无声纹时允许”。声纹不能替代身份认证或高风险操作授权，需要 Harness 凭据 TENCENT_SECRET_ID / TENCENT_SECRET_KEY。Tina 属于 Omni，专用 TTS 不支持；默认改用最接近其风格的 Chelsie。桌面壳当前禁用麦克风，点击话筒会在外部浏览器打开同一会话。"
						]
					})
				] })]
			});

window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function hasCustomPrefs(prefs) {
			return prefs.provider !== "qwen" || prefs.qwenWorkspaceId !== "" || prefs.qwenModel !== DEFAULTS.qwenModel || prefs.qwenVoice !== DEFAULTS.qwenVoice || prefs.qwenAsrModel !== DEFAULTS.qwenAsrModel || prefs.qwenTtsModel !== DEFAULTS.qwenTtsModel || prefs.qwenTtsVoice !== DEFAULTS.qwenTtsVoice || prefs.qwenVadThreshold !== DEFAULTS.qwenVadThreshold || prefs.qwenSilenceMs !== DEFAULTS.qwenSilenceMs || prefs.qwenMergeMs !== DEFAULTS.qwenMergeMs || prefs.openaiModel !== DEFAULTS.openaiModel || prefs.openaiVoice !== DEFAULTS.openaiVoice || prefs.instructions !== DEFAULTS.instructions;
		}
		/**
...
`.trim();
		...
		var QwenPipelineConnection = class {
			...
			async speak(text) {
				...
				for (const chunk of chunks) await new Promise((resolve, reject) => {
		...
		function opened(socket) {
			return new Promise((resolve, reject) => {
				...
				socket.addEventListener("error", () => reject(/* @__PURE__ */ new Error("本地语音代理连接失败")), { once: true });
			});
		}
		...
		function splitForTts(text, maxWeight = 1e3) {
			const spoken = text.replace(/```[\s\S]*?```/g, " ").replace(/\[([^\]]+)\]\([^)]+\)/g, "$1").replace(/https?:\/\/\S+/g, " ").replace(/[`*_#>|]/g, " ").replace(/[ \t]+/g, " ").replace(/\n{3,}/g, "\n\n").trim();
			if (spoken === "") return [];
		...
		*/
		var VoiceSummaryStream = class {
			...
			push(delta) {
				...
				if (!this.started) {
					...
					this.started = true;
					this.input = this.input.slice(start + 20);
				}
				this.drain(false);
			}
			finish(finalText) {
		...
		function extractVoiceSummary(text) {
			...
			if (start >= 0) {
				...
				return cleanSpeechText(takeWeighted(text.slice(bodyStart, end < 0 ? void 0 : end), MAX_SUMMARY_WEIGHT));
			}
			const paragraph = text.replace(/```[\s\S]*?```/g, " ").replace(/<!--[\s\S]*?-->/g, " ").trim().split(/\n\s*\n/)[0] ?? "";
		...
		function cleanSpeechText(text) {
			return text.replace(/```[\s\S]*?```/g, " ").replace(/`([^`]+)`/g, "$1").replace(/!??\[([^\]]*)\]\([^)]*\)/g, "$1").replace(/^#{1,6}\s+/gm, "").replace(/[>*_~]/g, "").replace(/\s+/g, " ").trim();
		}
		...
		var VoiceController = class {
			...
			async bufferTranscript(source, transcript, capturedWhileBusy = false) {
				...
				const wasBusy = capturedWhileBusy || this.hasPendingDraft() || this.taskAbort !== void 0 || this.turns.phase !== "listening" && this.turns.phase !== "endpoint-candidate";
				...
				this.transcriptTimer = setTimeout(() => {
					...
					if (combined !== "") this.turns.enqueue(() => this.composerOnly ? this.stageComposerTranscript(source, combined) : wasBusy ? this.handleBusyTranscript(source, combined) : this.handleTranscript(source, combined));
			...
			async finishObservedTurn(harnessTurn, result) {
				...
				try {
					...
					if (this.connection !== source || !this.turns.isCurrent(turnId)) return;
					this.setTurnPhase(turnId, "listening");
					this.setState("listening", isBargeInError(observed.speechError) ? "播报已打断；识别文字保留在输入框" : "Harness 已完成；继续说会写入输入框");
				} catch (error) {
			...
			async runHarnessTurn(source, task, turnId, taskAbort) {
				...
				const result = await this.bridge.delegate(this.sessionId, task, taskAbort.signal, {
					...
					onTextDelta: (delta) => summary.push(delta)
				});
				if (this.taskAbort === taskAbort) this.taskAbort = void 0;
				if (this.connection !== source || !this.turns.isCurrent(turnId)) return;
				if (!result.ok) {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
						...
						children: [
							...
							"。千问识别结果统一写入原生输入框，点击发送后才由 Harness 推理并播报；不会生成语音专属的“调整方向”任务。Tina 属于 Omni，专用 TTS 不支持；默认改用最接近其风格的 Chelsie。桌面壳当前禁用麦克风，点击话筒会在外部浏览器打开同一会话。"
						]
					})
				] })]
			});

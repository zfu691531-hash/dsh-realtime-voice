...
export class VoiceprintCapture {
  ...
  private active?: { itemId: string; frames: Int16Array[]; samples: number }
  private readonly completed = new Map<string, Int16Array>()
  ...
  push(frame: Int16Array): void {
    ...
    while (this.preRollSamples > PRE_ROLL_SAMPLES && this.preRoll.length > 1) {
      ...
      if (removed !== undefined) this.preRollSamples -= removed.length
    }
    const active = this.active
    if (active === undefined || active.samples >= MAX_SAMPLES) return
  ...
  stop(itemId: string): void {
    ...
    while (this.completed.size > 4) this.completed.delete(this.completed.keys().next().value as string)
  }
  ...
  discard(itemId: string): void {
    ...
    if (this.active?.itemId === itemId) this.active = undefined
  }
...
export async function getVoiceprintStatus(): Promise<VoiceprintStatus> {
  try {
    ...
    const body = await response.json() as { voiceprint?: Partial<VoiceprintStatus> }
    return {
...
export async function checkVoiceprint(operation: VoiceprintOperation, audio: string): Promise<VoiceprintGateResult> {
  try {
    ...
    const body = await response.json().catch(() => ({})) as { enrolled?: boolean; approved?: boolean; score?: number; error?: string }
    if (!response.ok) return { status: 'unavailable', error: body.error ?? `HTTP ${response.status}` }
    if (operation === 'enroll') return body.enrolled === true
      ? { status: 'enrolled' }
...
export async function deleteVoiceprint(): Promise<{ ok: true } | { ok: false; error: string }> {
  try {

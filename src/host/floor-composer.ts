...
export async function composeFloorText(
  input: FloorComposeRequest,
  key: string,
  signal: AbortSignal,
): Promise<string> {
  ...
  if (input.provider === 'qwen') {
    if (!WORKSPACE_RE.test(input.workspaceId) || !QWEN_MODELS.has(input.model)) throw new Error('invalid floor composer configuration')
    ...
    if (!response.ok) throw new Error(`floor provider rejected request (${response.status})`)
    const body = await response.json() as { choices?: Array<{ message?: { content?: unknown } }> }
    raw = body.choices?.[0]?.message?.content
  } else {
    if (!OPENAI_MODELS.has(input.model)) throw new Error('invalid floor composer configuration')
    ...
    if (!response.ok) throw new Error(`floor provider rejected request (${response.status})`)
    const body = await response.json() as { output_text?: unknown; output?: Array<{ content?: Array<{ type?: string; text?: unknown }> }> }
  ...
  if (cue === undefined) throw new Error('floor provider returned unsafe text')
  return cue
}

export function cleanFloorTopic(value: unknown): string {
...
export function validateFloorCue(value: unknown): string | undefined {
  if (typeof value !== 'string') return undefined
  const text = value.trim().replace(/\s+/g, ' ')
  if (text.length < 3 || text.length > 60 || /[\r\n]/.test(value)) return undefined
  if (/https?:|www\.|@|```|[#*_{}[\]<>]|(?:sk|token|key)-|Bearer/i.test(text)) return undefined
  if (/^(?:\{|\[)|(?:答案|结果|结论)(?:是|为)|已经(?:查到|完成|处理好)/.test(text)) return undefined
  const first = text.match(/^.*?[。！？!?]/)?.[0] ?? text
  return first.length >= 3 && first.length <= 60 ? first : undefined
}

function systemInstruction(stage: FloorStage, previous: string[]): string {
  const stageRule: Record<FloorStage, string> = {
